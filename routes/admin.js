...
router.post('/products', requireAdmin, upload.array('images', 6), async (req, res, next) => {
  try {
    const { name, description, basePrice, category, tags, featured, active } = req.body;
    ...
    await Product.create({
      name, description, basePrice: +basePrice, slug, sizes, images,
      category: category || undefined,
...
router.post('/categories', requireAdmin, async (req, res, next) => {
  try {
    const { name, order } = req.body;
    await Category.create({ name, slug: slugify(name), order: +order || 0 });
    res.redirect('/admin/categories');
  } catch (err) {
    const categories = await Category.find().sort({ order: 1 });
    res.render('admin/categories', { title: 'الفئات', categories, error: err.message });
